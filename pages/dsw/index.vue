      ...
      <div class="row">
        <div class="col-lg-12">
          <div class="text_box_one">
            <p v-html="dsws.overview"></p>
          </div>
        </div>
        <div class="col-lg-12 mt-5">
          <div class="text_box_two">
            <p v-html="dsws.function"></p>
          </div>
        </div>
      </div>
...
export default {
  ...
  layout: 'HomeLayout',
  asyncData({ store }) {
    store.dispatch('dsw/getRulesBanner')
    store.dispatch('dsw/getDsws')
  },
  data() {
    return {
      singleNewsDetails: '',
  ...
  computed: {
    ...mapGetters('dsw', ['events', 'topics', 'banner', 'dsws']),
  ...
  methods: {
    onPageChange(page) {
      this.$store.dispatch('dsw/allEventList', page)
    },
    ...
    filterByEvent() {
      ...
      this.$store.dispatch('dsw/filterByEvent', filterData).then((res) => {
        this.filter = !this.filter
      })
    },
    ...mapActions('dsw', ['topicList']),
